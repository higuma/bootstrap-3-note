...
> 
> > 最近は「障がい者」という表現が多い。配慮はもちろん理解できる。しかし何か表現の自由を剥奪しようとする(悪しき?)意思のようなものが...
...
> 
> 流れとしてはまずstatic埋め込みエンジンのLiquidがあり、それを利用したサイト構築ツールがJekyll、Jekyllをベースとしてそれをさらにブログに特化したのがOctpress。
...
冒頭の説明でNavbarは"resiponsive"なmata componentという記述があることに注意。このexample navbarは幅を狭めていくとある時点(xs境界付近)でバーが'collapse'してモバイル仕様の縦メニューに切り替わる。
...
それよりある程度ソースを読めるようになっておいた方がいい(結局ひと通り目は通した)。ソースは末尾のコロンがない書き方で最初は驚いたが、少し調べてみたら実はこれが「最新」らしい。まず次を参照。確かにセミコロンは使うなと書いてある。
...
確かにセミコロンなしで問題を生じる箇所だけ次行の先頭に何か置けばいいのというのは確かにうなずける(いちいち書く手間が省ける)。また最近は「セミコロン省略 + カンマ行頭」という人(npmの作者)もいるというのにはちょっと感心した。

> ちなみに自分はもうCoffeeScriptしか使わなくなってしまったので、こういうことには無頓着だった。

プラグインのソースに話を戻す。書き方は美しく統一されておりとても読みやすい。button.jsを例に構造を説明する。

* 先頭の`+function ($) {`の`+`はたぶん何かの対策(だが理由は不明)
* クラス定義
...
これはCSS transitionのサポート有無を判定し、サポートしていない場合はemulatorコードを用いるためのモジュール(他のモジュールがこれを利用する)。これは内部用なのでスキップする。
...
ここで$(...).button(...)はBootstrapのbuttonモジュール(jQueryプラグイン)。後ろの
Methodsの節に説明がある。上記コードで行っていることは次の通り。
...
> このページは開発者が「こういうこともできます」というデモみたいなものだと思った方がいいかも知れない。また個々を見ればBootstrap全体の構成を把握できるので、そういう意味でたとえカスタマイズする気がなくても一度目を通すといい。
